...
const PostCard = ({ post }) => {
  ...
  const handleLike = async () => {
    try {
      ...
      const res = await axios.get(
        `https://food-delivery-backend-gray.vercel.appapi/v1/post/${post._id}/${action}`,
        { withCredentials: true }
      );
  ...
  const handleComment = async () => {
    ...
    try {
      const res = await axios.post(
        `https://food-delivery-backend-gray.vercel.appapi/v1/post/${post._id}/comment`,
        { text: commentText },
        {
          headers: { "Content-Type": "application/json" },
          withCredentials: true,
  ...
  const handleDeletePost = async () => {
    try {
      const res = await axios.delete(
        `https://food-delivery-backend-gray.vercel.appapi/v1/post/delete/${post._id}`,
        { withCredentials: true }
      );
