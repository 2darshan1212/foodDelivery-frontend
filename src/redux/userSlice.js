  ...
  async (_, { rejectWithValue }) => {
    try {
      const res = await axios.get(
        "https://food-delivery-backend-gray.vercel.appapi/v1/user/followings",
        {
  ...
  async (targetUserId, { rejectWithValue }) => {
    try {
      const res = await axios.post(
        `https://food-delivery-backend-gray.vercel.appapi/v1/user/followorunfollow/${targetUserId}`,
        {},
        { withCredentials: true }
      );
...
export const getUserStats = createAsyncThunk(
  ...
  async (userId, { rejectWithValue }) => {
    try {
      const res = await axios.get(
        `https://food-delivery-backend-gray.vercel.appapi/v1/user/stats/${userId}`,
        { withCredentials: true }
      );
      return res.data;
    } catch (err) {
